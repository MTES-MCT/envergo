"""Split the Sites inscrits criteria into per hedge category evaluators.

Until now, a single evaluator carried the Sites inscrits criterion for every
hedge category. That evaluator is now tripled, one per category, with the same
result:
...
def split_sites_inscrits(apps, schema_editor):
...
def merge_sites_inscrits(apps, schema_editor):
    Criterion = apps.get_model("moulinette", "Criterion")
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(split_sites_inscrits, merge_sites_inscrits),
    ]
