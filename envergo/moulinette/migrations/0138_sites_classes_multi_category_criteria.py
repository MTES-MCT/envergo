"""Split the Sites classés criterion into per hedge category evaluators.

Until now, a single evaluator carried the Sites classés criterion for every
hedge category. It is now tripled, one per category, with the same result:
...
def split_sites_classes(apps, schema_editor):
...
def merge_sites_classes(apps, schema_editor):
    Criterion = apps.get_model("moulinette", "Criterion")
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(split_sites_classes, merge_sites_classes),
    ]
