...
class Natura2000Haie(CriterionEvaluator):
    ...
    def get_catalog_data(self):
        ...
        all_hedges = MultiLineString([h.geos_geometry for h in hedges], srid=EPSG_WGS84)

        hors_alignement = MultiLineString(
            [h.geos_geometry for h in hedges if h.hedge_type != "alignement"],
            srid=EPSG_WGS84,
        )

        alignement = MultiLineString(
            [h.geos_geometry for h in hedges if h.hedge_type == "alignement"],
            srid=EPSG_WGS84,
        )
        ...
        qs = (
            ...
            .filter(geometry__intersects=all_hedges)
            .annotate(hors_alignement=Intersection(hors_alignement, "geometry"))
            .annotate(alignement=Intersection(alignement, "geometry"))
        )

        l_n2000_hors_aa = 0.0
        l_n2000_aa = 0.0
        for zone in qs:
            l_n2000_hors_aa += zone.hors_alignement.transform(
                EPSG_LAMB93, clone=True
            ).length
            l_n2000_aa += zone.alignement.transform(EPSG_LAMB93, clone=True).length
