...
with open(Path(__file__).resolve().parent / "data" / "demarche_fake.json", "r") as file:
    DEMARCHES_SIMPLIFIEES_FAKE_DEMARCHE = file.read()

with open(Path(__file__).resolve().parent / "data" / "dossier_fake.json", "r") as file:
...
class DsClient:
    def __init__(self, ds_queries_filename="ds_queries.gql"):
        ...
        with open(
            Path(__file__).resolve().parent / "graphql" / ds_queries_filename
        ) as query_file:
            self.query = query_file.read()

    def launch_graphql_query(self, operation_name, variables=None) -> dict:
        """Launch graphql query on démarches simplifiées API"""
        headers = {
        ...
        if not settings.DEMARCHES_SIMPLIFIEES["ENABLED"]:
            raise NotEnabledException(
                f"Demarches Simplifiees is not enabled. Doing nothing and use fake dossier."
