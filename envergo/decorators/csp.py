...
# WARNING This is not part of the django 6.0 backport
def _make_csp_update_decorator(config_attr_name, config_attr_value):
    """CSP update decorator factory.

    This creates a decorator that updates existing csp headers with new values.
    """
    ...
    def decorator(view_func):
        ...
        async def _wrapped_async_view(request, *args, **kwargs):
            response = await view_func(request, *args, **kwargs)
            config = getattr(response, config_attr_name, {})
            config.update(config_attr_value)
            setattr(response, config_attr_name, config)
            return response
...
def csp_update(config):
    """Update the Content-Security-Policy header for a view."""
    return _make_csp_update_decorator("_csp_config", config)


def csp_report_only_update(config):
    """Update the Content-Security-Policy-Report-Only header for a view."""
