...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        events = Event.objects.filter(metadata__lat__isnull=False, metadata__lng__isnull=False).exclude(metadata__has_key='commune')
        for event in events:
            commune = get_commune_from_coords(event.metadata['lng'], event.metadata['lat'])
