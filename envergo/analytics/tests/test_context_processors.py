...
from envergo.analytics.tests.factories import SimulationEventFactory, UsageEventFactory
...
def test_single_usage_event(req):
    UsageEventFactory(session_key=VISITOR_ID)
    context = analytics(req)
    assert context == {"matomo_dimensions": [(1, "n1_d0"), (2, "n0_d0")]}


def test_several_usage_events(req):
    UsageEventFactory.create_batch(15, session_key=VISITOR_ID)
    context = analytics(req)
    assert context == {"matomo_dimensions": [(1, "n2_d0"), (2, "n0_d0")]}


def test_single_usage_old_event(req):
    UsageEventFactory(
