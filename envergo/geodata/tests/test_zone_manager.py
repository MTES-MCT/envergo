...
class TestFindCovering:
    ...
    def test_single_point_in_zone(self):
        ...
        result = Zone.objects.find_covering(
            {"p1": POINT_IN_A}, MAP_TYPES.zonage, "44"
        )
        assert "p1" in result
    ...
    def test_wrong_department_returns_empty(self):
        ...
        result = Zone.objects.find_covering(
            {"p1": POINT_IN_A}, MAP_TYPES.zonage, "99"
        )
        assert result == {}
    ...
    def test_returns_zone_objects(self):
        """Returned values are Zone model instances."""
        make_zonage_map([
            (MultiPolygon([ZONE_A_POLY]), {"identifiant_zone": "A"}),
        ])
        result = Zone.objects.find_covering(
        ...
        assert isinstance(result["p1"], Zone)
...
class TestFindNearest:
    ...
    def test_point_inside_zone_returns_that_zone(self):
        ...
        result = Zone.objects.find_nearest(
            POINT_IN_A, MAP_TYPES.zonage, "44", 50_000
        )
    ...
    def test_nearby_point_within_cap(self):
        ...
        result = Zone.objects.find_nearest(
            nearby, MAP_TYPES.zonage, "44", 50_000
        )
        assert result is not None
        assert result.pk == zones[0].pk
    ...
    def test_returns_closest_when_multiple_zones(self):
        ...
        result = Zone.objects.find_nearest(
            between, MAP_TYPES.zonage, "44", 50_000
        )
        assert result is not None
    ...
    def test_wrong_department_returns_none(self):
        ...
        result = Zone.objects.find_nearest(
            POINT_IN_A, MAP_TYPES.zonage, "99", 50_000
        )
        assert result is None

    def test_returns_zone_object(self):
        """Returned value is a Zone model instance."""
        make_zonage_map([
            (MultiPolygon([ZONE_A_POLY]), {"identifiant_zone": "A"}),
        ])
        result = Zone.objects.find_nearest(
