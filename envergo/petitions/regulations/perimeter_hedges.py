...
def get_hedges_length_in_zones(perimeter, hedges):
    """Return `hedges`, with the length of their intersection with the perimeter instead of their full geometry length.
...
def get_regulation_hedges_length_in_perimeter(moulinette, regulation_slug):
    ...
    Each hedge's length is reduced to its intersection with the perimeter.
    """
    hedges = HedgeList()
