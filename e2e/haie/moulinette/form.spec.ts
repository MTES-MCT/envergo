...
test('A petitioner can submit a project', async ({ page }) => {
    ...
    await page.locator('#hedge-input-iframe').contentFrame().getByLabel('Description de la haie D2').getByText("Mare ou point d'eau à moins de 500 m").click();
    ...
    await expect(page.getByText('autorisation est prête à être complétée')).toBeVisible();
    await expect(page.getByText('La plantation envisagée est adéquate')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Déposer une demande ' })).toBeVisible();
