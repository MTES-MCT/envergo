...
test('A petitioner can submit a project', async ({ page }) => {
    ...
    await page.locator('#hedge-input-iframe').contentFrame().getByRole('dialog', { name: 'Description de la haie P1' }).getByLabel("Alignement d'arbres").check();
    ...
    await page.locator('#hedge-input-iframe').contentFrame().getByRole('dialog', { name: 'Description de la haie P2' }).getByLabel('Haie mixte').check();
